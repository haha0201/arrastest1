...
class Skill {
    ...
    update() {
        ...
        this.dam = apply(3, attrib[skcnv.dam]) * 2;
        this.spd = 0.5 + apply(1.5, attrib[skcnv.spd]) * 1.5;

        this.acl = apply(0.5, attrib[skcnv.rld]) * 2;
        
        this.rst = 0.5 * attrib[skcnv.str] + 2.5 * attrib[skcnv.pen];
        this.ghost = attrib[skcnv.pen];
        
        this.shi = c.GLASS_HEALTH_FACTOR * apply(3 / c.GLASS_HEALTH_FACTOR - 1, attrib[skcnv.shi]) / 2;
        this.atk = apply(1, attrib[skcnv.atk]);
        this.hlt = c.GLASS_HEALTH_FACTOR * apply(2 / c.GLASS_HEALTH_FACTOR - 1, attrib[skcnv.hlt]) / 3;
        this.mob = apply(0.8, attrib[skcnv.mob]) * 4; 
        this.rgn = apply(25, attrib[skcnv.rgn]) / 2;
...
class Entity {
    ...
    contemplationOfMortality() {
        ...
        if (this.isDead()) {
            ...
            let killText = (notJustFood) ? '' : "Oof, you got Utterly Decimated by ",
            ...
            if (notJustFood) {
                killers.forEach(instance => {
                    if (instance.master.type !== 'food' && instance.master.type !== 'crasher') {
                        killText += (instance.name == '') ? (killText == '') ? 'A crasher or a polygon' : 'a crasher or a polygon' : instance.name;
                        killText += ' and ';
                    }
                    // Only if we give messages
                    if (dothISendAText) { 
                        instance.sendMessage('You destroyed the noob tank by the name of ' + name + ((killers.length > 1) ? ' (with some help).' : '.')); 
                    }
                });
                // Prepare the next part of the next 
                killText = killText.slice(0, -4);
                killText += 'utterly decimated you with ';
            }
            // Broadcast
            ...
            if (killText === 'You have been kille') killText = 'Wow, that death was... uhh... stupid.';
            this.sendMessage(killText + '.');
            // If I'm the leader, broadcast it:
            if (this.id === room.topPlayerID) {
                let usurptText = (this.name === '') ? 'The leader': this.name;
                if (notJustFood) { 
                    usurptText += ' is a noob, and was decimated by ';
                    killers.forEach(instance => {
                        usurptText += ' ';
                        usurptText += (instance.name === '') ? 'a mysterious noob' : instance.name;
                        usurptText += ' and';
                    });
                    usurptText = usurptText.slice(0, -4);
                    usurptText += '!';
                } else {
                    usurptText += ' has just died, we think its because a Polygon, but who cares';
                }
...
var maintainloop = (() => {
    ...
    function placeRoids() {
        ...
        // Start placing them
        let roidcount = room.roid.length * room.width * room.height / room.xgrid / room.ygrid / 250000 / 1.5;
        let rockcount = room.rock.length * room.width * room.height / room.xgrid / room.ygrid / 250000 / 1.5;
        let count = 0;
        for (let i=Math.ceil(roidcount*0.55); i; i--) { count++; placeRoid('roid', Class.obstacle); }
        for (let i=Math.ceil(roidcount*0); i; i--) { count++; placeRoid('roid', Class.babyObstacle); }
        for (let i=Math.ceil(rockcount*0.55); i; i--) { count++; placeRoid('rock', Class.obstacle); }
    ...
    let spawnBosses = (() => {
        ...
        let boss = (() => {
            let i = 0,
                ...
                bois = [Class.egg],
                n = 0,
                begin = 'Things are arriving. Beware.',
                arrival = 'The stuffs have arrived.',
                loc = 'norm';
            let spawn = () => {
                let spot, m = 0;
                do {
            ...
            return {
                prepareToSpawn: (classArray, number, nameClass, typeOfLocation = 'norm') => {
                    ...
                    i = 0;
                    if (n === 1) {
                        begin = 'Stuffz are comin bro.';
                        arrival = names[0] + ' has destroyed our peace and quiet.'; 
                    } else {
                        begin = 'Idiot noob AI tanks are about to spawn - get ready...';
                        arrival = '';
                        for (let i=0; i<n-2; i++) arrival += names[i] + ', ';
                        arrival += names[n-2] + ' and ' + names[n-1] + ' are both idiots cause they spawned :O';
                    }
                },
                spawn: () => {
        ...
        return census => {
            if (timer > 60 && ran.dice(160 - timer)) {//reduce values here to shorten boss spawn timing :)
    ...
    let makenpcs = (() => {
        // Make base protectors if needed.
           let g = (loc, team) => { 
                let o = new Entity(loc);
                    o.define(Class.baseobstacle);
                    o.team = -team;
                    o.color = [10, 11, 12, 15][team-1];
            };
            for (let i=1; i<5; i++) {
                room['bas' + i].forEach((loc) => { g(loc, i); }); 
            }
           let f = (loc, team) => { 
                let o = new Entity(loc);
        ...
        return () => {
            ...
            spawnBosses(census);
                ...
                // Slowly upgrade them
                bots.forEach(o => {
                    if (o.skill.level < 400) {
                        o.skill.score += 100000000;
                        o.skill.maintain();
                    }
                });
        };
